...
class Token:
    ...
    def __repr__(self):
        # Adding Digit, Letter, Keyword, Comma
        #if(self.type )
...
class Parser:
    ...
    # Need to define Query, IDList, CondList, Cond, Term
...
# print("\n Testcase 1.")
# parser = Parser ("FROM C1, C2 SELECT T1 WHERE C1 AND C2 > 5.23") # shouldnt work
# parser.run()

# print("\n Testcase 2.") #expecting Keyword saw ID
# parser = Parser ("SELECT C1, C2 SELECT T1 FROM C1 < 5.23 AND C2 = 5") # 2 select statements
# parser.run()

# print("\n Testcase 3.") #expecting Keyword saw ID
# parser = Parser ("SELECT C1, C2 WHERE T1 > 5.23 AND C2 = 5 FROM C1 ")
# parser.run()

# print("\n Testcase 4.") #expecting Keyword saw ID
# parser = Parser ("WHERE C1, C2 FROM T1 SELECT C1 > 5.23 AND C2 = 5") # not work.
# parser.run()

# print("\n Testcase 5.") #expecting Keyword saw ID
# parser = Parser ("FROM C1, C2 SELECT T1 WHERE C1 > 5.23 AND C2 = 5")
# parser.run()
